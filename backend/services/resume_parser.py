...
# python-magic requires libmagic which causes a hard crash (segfault) on
# Windows if the DLL is missing — a plain try/except cannot catch that.
# Only attempt the import on platforms where it is known to work.
_HAS_MAGIC = False
...
# Mapping file extensions to the short type names used internally
_EXT_TO_TYPE = {
    '.pdf': 'pdf',
    '.doc': 'doc',
    '.docx': 'docx',
...
def validate_file(file_data:bytes, filename:str)->Tuple[bool, str, Optional[str]]:
    ...
    # Try python-magic first for accurate MIME type detection
    if _HAS_MAGIC:
        try:
            mime_type=magic.from_buffer(file_data, mime=True)
            if mime_type in SUPPORTED_MIME_TYPES:
                return True, '', SUPPORTED_MIME_TYPES[mime_type]
            # If magic detected an unsupported MIME, still fall back to extension check below
        except Exception as e:
            log_warning(f"python-magic detection failed, falling back to extension: {e}", context='validate_file')
    
    # Fallback: use file extension
    ext = os.path.splitext(filename)[1].lower() if filename else ''
    if ext in _EXT_TO_TYPE:
        return True, '', _EXT_TO_TYPE[ext]
    
    supported = ', '.join(SUPPORTED_EXTENSIONS).upper()
    return False, (
