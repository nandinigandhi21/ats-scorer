# Feedback engine: detects specific issues in a resume and generates actionable feedback.
...
def analyze_issues(
    ...
    if len(description_lines) >= 4 and len(action_verbs) < 7:
        detected.append(IssueDetail(
    ...
    achievement_count = sum(len(re.findall(p, experience_text, re.IGNORECASE)) for p in number_patterns) if experience_text else 0

    if experience_text and achievement_count == 0:
        detected.append(IssueDetail(
    ...
    elif experience_text and achievement_count < 4:
        detected.append(IssueDetail(
            issue_title="Few Quantifiable Achievements",
            severity_level="Low",
            ats_impact="Medium",
            explanation=(
                f"Your experience section only has {achievement_count} quantifiable achievements. "
                "Resumes with 4 or more quantified impact metrics are significantly more likely "
                "to stand out to recruiters and pass ATS evaluation screens."
            ),
            where_it_appears="Experience section — bullet point content",
            how_to_fix=(
                "Add metrics (numbers, percentages, time saved, or revenue generated) to "
                "more of your bullet points to demonstrate the concrete impact of your work."
            ),
            action_items=[
                "Identify at least 2 more bullet points where you can estimate a metric",
                "Use ranges if exact numbers are unavailable (e.g. 'reduced load time by 30-40%')",
                "Specify scale where possible (e.g. 'handled a database of 100K+ records')"
            ],
            example_improvement=(
                "Before:\n"
                "• Worked on database optimization tasks\n\n"
                "After:\n"
                "• Optimized SQL query index execution, reducing page load latency by 35% on a dataset of 100K+ rows"
            ),
        ))
    ...
    if formatting_score < 10:
    ...
    elif 10 <= formatting_score < 20:
        missing_sections = []
        if not exp_entries: missing_sections.append("Work Experience")
        if not edu_entries: missing_sections.append("Education")
        if not parsed_resume.get('skills'): missing_sections.append("Skills")
        if not summary: missing_sections.append("Professional Summary")
        if not proj_entries: missing_sections.append("Projects")

        explanation_msg = (
            f"Your formatting score is {formatting_score}/20, which is good but has room for improvement. "
        )
        if missing_sections:
            explanation_msg += f"Missing or very short sections: {', '.join(missing_sections)}. "
        ...
        if bullet_count < 10:
            explanation_msg += f"You have {bullet_count} bullet points. ATS parsers prefer resumes with structured bullet points for experience and projects (10-15 bullet points recommended)."

        detected.append(IssueDetail(
            issue_title="Minor Resume Formatting Issues",
            severity_level="Low",
            ats_impact="Medium",
            explanation=explanation_msg,
            where_it_appears="Overall document structure and bullet points",
            how_to_fix=(
                "Ensure all standard sections are fully populated and use 10+ clear, "
                "consistent bullet points across your experience and projects."
            ),
            action_items=[
                "Verify that Experience, Projects, Education, and Skills have their own headers",
                "Ensure you have at least 10 bullet points starting with strong action verbs",
                "Use a clean single-column format with no text boxes, headers/footers, or graphics"
            ],
            example_improvement=(
                "Structure your resume like this:\n"
                "• Professional Summary (3-4 lines)\n"
                "• Technical Skills (grouped by category)\n"
                "• Experience (with clear bullet points)\n"
                "• Projects (with clear bullet points)\n"
                "• Education"
            )
        ))

    # 4b. Sub-optimal Keywords and Skills
    keywords_score = scores.get('keywords_score', 25)
    if keywords_score < 20:
        detected.append(IssueDetail(
            issue_title="Optimize Keywords and Skills",
            severity_level="Medium",
            ats_impact="High",
            explanation=(
                f"Your keyword/skill score is {keywords_score}/25. ATS systems filter candidates "
                "by counting the frequency and relevance of specific skills and terms matching "
                "the target job description. Having fewer than 15 skills or lacking key industry terms "
                "decreases your visibility."
            ),
            where_it_appears="Skills section and experience/project descriptions",
            how_to_fix=(
                "Expand your Skills section to include 12-18 relevant skills and ensure "
                "key industry terminologies (matching your target roles) are integrated naturally "
                "into your bullet points."
            ),
            action_items=[
                "Add 5-10 more technical skills relevant to your target role",
                "Ensure skills are written in the standard industry formats (e.g. 'React.js', 'PyTorch', 'REST APIs')",
                "If a Job Description is provided, make sure to add missing keywords highlighted in the analysis"
            ],
            example_improvement=(
                "Instead of listing just 'Python', list: 'Python (FastAPI, Pandas, NumPy, Scikit-Learn)'\n"
                "Incorporate keywords into descriptions: 'Optimized PostgreSQL database queries to improve search performance by 35%.'"
            )
        ))
