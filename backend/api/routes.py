...
async def analyze_resume(
    ...
    try:
        ...
        # parse_resume_file is synchronous (pdfplumber/magic are blocking I/O).
        # Run it in a thread so the event loop stays free.
        loop = asyncio.get_event_loop()
        resume_text, _metadata = await loop.run_in_executor(
            None,
            partial(parse_resume_file, file_bytes, filename),
        )
    ...
    #Full Analysis Pipeline — run in a thread pool so the async event loop is not blocked
    try:
...
async def generate_pdf(
    ...
    try:
        ...
        pdf_bytes = generate_combined_pdf(html_docs, raw_data=data.model_dump())
...
async def generate_history_pdf(
    ...
    try:
        ...
        pdf_bytes = generate_combined_pdf(html_docs, raw_data=analysis_data)
